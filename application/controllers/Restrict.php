...
public function ajax_save_course(){
		...
		if(empty($data["course_duration"])){
			...
			if (!($data["course_duration"] > 0 && $data["course_duration"] < 100)){
				$json["error_list"]["#course_duration"] = "Duração do curso deve ser maior que zero horas e maior que 100!";
			}
		}
	...
	public function ajax_save_service(){
		...
		if(empty($data["service_name"])){
			...
			if ($this->services_model->is_duplicated("service_name", $data["service_name"], $data["service_id"])){
				$json["error_list"]["#service_name"] = "Nome do serviço já existente!";
			}
		}
...
public function ajax_save_user(){
		...
		if(empty($data["user_email"])){
					$json["error_list"]["#user_mail"] = "E-mail é obrigatório!";
				} else {
					if ($this->users_model->is_duplicated("user_email", $data["user_email"], $data["user_id"])){
						...
						if ($data["user_email"] != $data["user_email_confirm"]){
							...
							$json["error_list"]["#user_email"] = "E-mails não conferem!";
						}
					}
				}
				...
				if(empty($data["user_password"])){
					$json["error_list"]["#user_password"] = "A senha é obrigatória!";
						...
						if ($data["user_password"] != $data["user_password_confirm"]){
							...
							$json["error_list"]["#user_password_confirm"] = "As senhas não conferem!";
						}
				}
	...
	/*Possível problema de carregamento da datatable*/
	...
	public function ajax_get_delete_course_data(){
	...
	public function ajax_get_delete_service_data(){
	...
	public function ajax_get_delete_member_data(){
		...
		public function ajax_get_delete_user_data(){
	...
	public function ajax_list_course(){
		...
		foreach ($services as $service) {
			...
			//$row[] = $service->service_img;
			

			if($service->service_img){
				$row[] = '<img src="' . base_url().$service->service_img.'" style="max-height: 100px; max-width: 100px;">';
			} else{
				$row[] = "";
			}
